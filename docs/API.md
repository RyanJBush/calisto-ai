...
Interactive OpenAPI docs (Swagger UI) are served at
`http://localhost:8000/docs` when the backend is running, and a raw OpenAPI
schema is available at `http://localhost:8000/openapi.json`.

The backend currently exposes **15 endpoints** grouped under five routers:
`auth`, `documents`, `chat`, `admin`, and `health`. The most useful ones for a
recruiter walkthrough are documented below; the rest are visible in `/docs`.

> Note: the `answer_mode` field returned by `/api/chat/query` is always
> `grounded_heuristic` in the current build — answers are composed from the
> top retrieved chunks by a deterministic heuristic, not generated by an LLM.
> See [ARCHITECTURE.md](ARCHITECTURE.md) for the swap-in seams.
